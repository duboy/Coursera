...
S = {-4, -2, 1, 2, 5, 0}
# Replace [ ... ] with a one-line list comprehension in which S appears
exclude_zero_list = [(i,j,k) for i in S for j in S for k in S if i+j+k == 0 if i!=j!=k!=0]
...
first_of_tuples_list = [(i,j,k) for i in S for j in S for k in S if i+j+k == 0 if i!=j!=k!=0][0]
...
representation_dict = {n:(n//base**2 % base, n//base%base, n%base) for n in range(base**3)}
...
listdict2dict = {n:i for (n, i) in zip(names, id2salary.values())}
